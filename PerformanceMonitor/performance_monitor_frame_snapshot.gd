class_name PerformanceMonitor_FrameSnapshot

var frame_number: int
var start_usec: int
var end_usec: int
var total_duration_usec: int
var areas: Array[PerformanceMonitor_AreaTiming] = []
var deferred_calls: Array[PerformanceMonitor_DeferredCallTiming] = []

# Frame timing breakdown (from _engine_track_frame_times)
var game_logic_usec: int = 0
var draw_usec: int = 0
var post_render_usec: int = 0

# Detailed game logic breakdown
var navigation_sync_usec: int = 0
var physics_steps_usec: int = 0
var input_flush_usec: int = 0
var process_usec: int = 0
var message_queue_flush_usec: int = 0

# Additional frame components
var script_frame_usec: int = 0
var audio_update_usec: int = 0

func _init(p_frame_number: int, p_start_usec: int) -> void:
	self.frame_number = p_frame_number
	self.start_usec = p_start_usec
	self.end_usec = 0
	self.total_duration_usec = 0

func finalize(p_end_usec: int) -> void:
	self.end_usec = p_end_usec
	self.total_duration_usec = p_end_usec - self.start_usec

func add_area(area: PerformanceMonitor_AreaTiming) -> void:
	self.areas.push_back(area)

func get_area_count() -> int:
	return self.areas.size()

func _to_string() -> String:
	return "Frame #%d: %s (%d areas)" % [
		frame_number,
		PerformanceMonitor_AreaStats.format_duration(total_duration_usec),
		areas.size()
	]
